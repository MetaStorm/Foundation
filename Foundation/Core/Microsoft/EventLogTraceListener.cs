...
/*
 */
namespace Foundation.Core.Microsoft {
  using System;
  ...
  public class EventLogTraceListener2 : TraceListener {
    ...
    /// </devdoc>
    public EventLogTraceListener2(EventLog eventLog)
    ...
    /// </devdoc>
    public EventLogTraceListener2(string source) {
    ...
    public override void TraceData(TraceEventCache eventCache, String source, TraceEventType severity, int id, params object[] data) {
      ...
      WriteEventSafe(id, inst, logMessage);
    }

    protected void WriteEventSafe(int id, EventInstance inst,object logMessage) {
      try {
        eventLog.WriteEvent(inst, EventLogger.TrancateMessage(logMessage + "", 31839));
      } catch (Exception exc) {
        EventLogger.LogApplicationError(logMessage, id);
        EventLogger.LogApplicationError(new Exception(new { @event = new { eventLog.Source }, error = exc.Message } + Environment.NewLine + "\tError while writing to EventLog. See previouse entry." + Environment.NewLine, exc));
      }
    }

    protected EventInstance CreateEventInstance(System.Diagnostics.TraceEventType severity, int id) {
      ...
      EventInstance data = new System.Diagnostics.EventInstance(id, 0);
